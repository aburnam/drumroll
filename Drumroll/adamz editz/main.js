...
function drawMenu(){//drawing squares for modes
    if(phase == 0 || phase == 3){
        ...
        if(phase == 0){ 
            ...
            menuctx.fillText("Choose an", 1015, 160);
            menuctx.fillText("instrument", 1005, 220)
			menuctx.fillStyle = "cornflowerblue";
...
function makeBeat(buffer, beat){
	var fps = buffer.length / buffer.duration;
	  ...
	  context.decodeAudioData(data, function(buf) {
		...
		if(sample == false){
			if(firstbeat == false & finalsource.buffer != null){
				...
				var tmpbuf2 = makeBeat(buf, 0);
				finalsource = context.createBufferSource();
				finalsource.buffer = appendBuffer(tmpbuf, tmpbuf2);
			}
			else{
				finalsource.buffer = makeBeat(buf, 0);
			}
		}
		source.buffer = buf;
		source.loop = false;
